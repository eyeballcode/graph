export default class Heap {
  ...
  childComparison(parentIndex, childIndex) {
  ...
  largestChild(parentIndex) {
    ...
    if (left === this.getSize() || this._array[left] > this._array[right]) {
      return left
    }
    return right
  }
  ...
  sink(index) {
    ...
    while (2 * index <= this.getSize()) {
      ...
      if (this._array[index] > this._array[child]) break
  ...
  rise(index) {
    ...
    while (2 <= index && this._array[index] > this._array[Math.floor(index / 2)]) {
      // Swap down with the largest child and move the index pointer to its new position
